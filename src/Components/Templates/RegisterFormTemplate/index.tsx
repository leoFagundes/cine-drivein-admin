...
export default function RegisterFormTemplate({ label, inputs, buttonOnClick, buttonLabel, setItemWithSubitems, itemWithSubitems, withSubitem = false }: RegisterFormTemplateType) {
  ...
  useEffect(() => {
    const fetchAdditionalItems = async () => {
      try {
        const additionalItemsResponse = await AdditionalItemRepositories.getAdditionalItems();
  ...
  const handleDropdownChange = (event: ChangeEvent<HTMLSelectElement>, key: string) => {
    ...
    if (setItemWithSubitems) {
      setItemWithSubitems(prevItem => {
        ...
        if (key === 'general') {
          const isDuplicate = updatedItem.additionals.some(item => item.additionalItem === selectedValue);
          if (isDuplicate) {
          ...
          const isDuplicate = updatedItem.additionals_sauces.some(item => item.additionalItem === selectedValue);
          if (isDuplicate) {
            console.error("Este valor já foi selecionado.");
            return updatedItem;
          }
          updatedItem.additionals_sauces.push({ additionalItem: selectedValue });
        } else if (key === 'drinks') {
          const isDuplicate = updatedItem.additionals_drinks.some(item => item.additionalItem === selectedValue);
          if (isDuplicate) {
            console.error("Este valor já foi selecionado.");
            return updatedItem;
          }
          updatedItem.additionals_drinks.push({ additionalItem: selectedValue });
        } else if (key === 'sweets') {
          const isDuplicate = updatedItem.additionals_sweets.some(item => item.additionalItem === selectedValue);
          if (isDuplicate) {
            console.error("Este valor já foi selecionado.");
            return updatedItem;
          }
          updatedItem.additionals_sweets.push({ additionalItem: selectedValue });
        }
  ...
  const handleItemRemove = (key: string, selectedValue: string) => {
    if (setItemWithSubitems) {
      setItemWithSubitems(prevItem => {
        ...
        if (key === 'general') {
          updatedItem.additionals = updatedItem.additionals.filter(item => item.additionalItem !== selectedValue);
        } else if (key === 'sauces') {
          updatedItem.additionals_sauces = updatedItem.additionals_sauces.filter(item => item.additionalItem !== selectedValue);
        } else if (key === 'drinks') {
          updatedItem.additionals_drinks = updatedItem.additionals_drinks.filter(item => item.additionalItem !== selectedValue);
        } else if (key === 'sweets') {
          updatedItem.additionals_sweets = updatedItem.additionals_sweets.filter(item => item.additionalItem !== selectedValue);
        }
  ...
  return (
    <section className={styles.container}>
      <Text fontWeight='semibold' marginBottom="32px" fontSize='large'>{label}</Text>
                ...
                <Fragment key={key}>
                  {subitemStates[key] === true &&
                    <div className={styles.optionsContent}>
                      <Text fontColor='background-secondary-color' fontWeight='semibold'>Escolha as Opções <Text fontColor='placeholder-color'>({key})</Text></Text>
                      <Input
                        type="text"
                        placeholder="Search..."
                        value={searchTerm}
                        ...
                        border
                      />
                      <select className={styles.optionsSelect} value="" onChange={(e) => handleDropdownChange(e, key)} multiple>
                      ...
                      <div className={styles.items}>
                        <Text fontColor='background-secondary-color' fontWeight='semibold'>Opções Escolhidas</Text>
                        {subitemStates.general && itemWithSubitems?.additionals && renderAdditionals(itemWithSubitems.additionals, key)}
                        {subitemStates.sauces && itemWithSubitems?.additionals_sauces && renderAdditionals(itemWithSubitems.additionals_sauces, key)}
                        {subitemStates.drinks && itemWithSubitems?.additionals_drinks && renderAdditionals(itemWithSubitems.additionals_drinks, key)}
                        {subitemStates.sweets && itemWithSubitems?.additionals_sweets && renderAdditionals(itemWithSubitems.additionals_sweets, key)}
                      </div>
                    </div>
                  }
                </Fragment>
